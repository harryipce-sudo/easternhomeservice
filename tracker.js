...
function populateJobClientOptions(selected = "") { const clients = [...new Set(state.records.map((record) => String(record.customerName || "").trim()).filter((name) => name && name !== "-"))].sort((a, b) => a.localeCompare(b)); const select = $("#job-client"); select.innerHTML = `<option value="">Select a saved client</option>${clients.map((name) => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join("")}`; select.value = clients.includes(selected) ? selected : ""; }
