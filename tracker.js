...
function renderClientPortal() {
  ...
  $("#client-portal-summary").innerHTML = `<article class="report-card"><span>Client</span><strong>${client}</strong><p>Read-only invoice page</p></article><article class="report-card"><span>Unpaid invoices</span><strong>${invoices.length}</strong><p>Currently visible to the client</p></article><article class="report-card"><span>Client payment reports</span><strong>${clientReported.length}</strong><p>Do not update main payment status</p></article><article class="report-card"><span>Total amount (inc. GST)</span><strong>${currency(total)}</strong><p>All unpaid BRW Group invoices</p></article><article class="report-card"><span>Referral payable (inc. GST)</span><strong>${currency(referral)}</strong><p>Internal tracking only</p></article>`;
  $("#client-portal-body").innerHTML = invoices.map((record) => { const confirmation = confirmationByInvoice.get(String(record.job.invoiceNumber || "")); return `<tr><td>${escapeHtml(record.job.invoiceNumber || "—")}</td><td>${escapeHtml(record.address || "—")}</td><td class="detail">${escapeHtml(record.job.detail || "—")}</td><td>${confirmation?.paidDate ? `Reported ${escapeHtml(confirmation.paidDate)}` : "—"}</td><td class="detail">${escapeHtml(confirmation?.note || "—")}</td><td class="money">${referralPayable(record) ? currency(referralPayable(record)) : "—"}</td><td class="money">${currency(record.job.quote)}</td></tr>`; }).join("") || `<tr><td colspan="7">No unpaid BRW Group invoices.</td></tr>`;
}
