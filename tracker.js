...
function updateMarkupAmount() { const quote = Number($("#job-quote").value) || 0; const percentage = Number($("#job-markup").value) || 0; const amount = percentage > 0 ? quote - (quote / (1 + percentage / 100)) : 0; $("#job-markup-amount").value = amount.toFixed(2); }
...
function renderQuotes() {
  ...
  const needsBoardReview = records.filter((record) => record.job.invoiceNumber && record.job.payment !== "paid" && !["invoiced", "scheduled"].includes(record.job.status));
