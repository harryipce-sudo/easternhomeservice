...
function renderQuotes() {
  ...
  $("#invoice-summary").innerHTML = `<article class="invoice-metric pending"><span>Pending for invoice</span><strong>${pendingForInvoice.length} job${pendingForInvoice.length === 1 ? "" : "s"}</strong><b>${currency(pendingTotal)}</b></article><article class="invoice-metric unpaid"><span>Unpaid invoices</span><strong>${unpaid.length} job${unpaid.length === 1 ? "" : "s"}</strong><b>${currency(unpaidTotal)}</b></article><article class="invoice-metric paid"><span>Paid invoices</span><strong>${paid.length} job${paid.length === 1 ? "" : "s"}</strong><b>${currency(paidTotal)}</b></article><article class="invoice-metric unpaid"><span>Unpaid referral payable (inc. GST)</span><strong>${unpaidReferralCount} invoice${unpaidReferralCount === 1 ? "" : "s"}</strong><b>${currency(unpaidReferralTotal)}</b></article><article class="invoice-metric paid"><span>Paid referral payable (inc. GST)</span><strong>${paidReferralCount} invoice${paidReferralCount === 1 ? "" : "s"}</strong><b>${currency(paidReferralTotal)}</b></article><div class="invoice-payment-bar" aria-label="${unpaid.length} unpaid and ${paid.length} paid invoices"><span class="unpaid" style="width:${(unpaid.length / total) * 100}%"></span><span class="paid" style="width:${(paid.length / total) * 100}%"></span></div><div class="invoice-bar-legend"><span><i class="unpaid"></i>Unpaid: ${unpaid.length}</span><span><i class="paid"></i>Paid: ${paid.length}</span></div>`;
