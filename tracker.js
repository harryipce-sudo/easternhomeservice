...
function renderQuarterSummary(invoices, expenses) { const body = $("#quarter-body"); if (!body) return; body.innerHTML = [1,2,3,4].map((quarter) => { const quarterInvoices = invoices.filter((record) => profitQuarterFor(profitInvoiceDate(record)) === quarter); const quarterExpenses = expenses.filter((expense) => profitQuarterFor(expense.date) === quarter); const income = quarterInvoices.reduce((sum, record) => sum + Number(record.job.quote || 0) / 1.1, 0); const referral = quarterInvoices.reduce((sum, record) => sum + markupAmountExGst(record.job), 0); const totalExpenses = quarterExpenses.reduce((sum, expense) => sum + Number(expense.amount || 0), 0); const profit = income - referral - totalExpenses; return `<tr><td><strong>Q${quarter}</strong></td><td class="money">${currency(income)}</td><td class="money">${currency(referral)}</td><td class="money">${currency(totalExpenses)}</td><td class="money"><strong>${currency(profit)}</strong></td></tr>`; }).join(""); }
function renderProfit() { const select = $("#profit-fy"); if (!select) return; const paidInvoices = activeRecords().filter((record) => record.job.status === "invoiced" && record.job.payment === "paid"); const expenses = profitExpenses(); const years = [...new Set([currentFinancialYear(), ...paidInvoices.map(profitInvoiceDate).map(financialYearFor), ...expenses.map((expense) => financialYearFor(expense.date))].filter(Boolean))].sort().reverse(); if (!years.includes(state.profitFY)) state.profitFY = years.includes(currentFinancialYear()) ? currentFinancialYear() : years[0]; select.innerHTML = years.map((year) => `<option value="${escapeHtml(year)}"${year === state.profitFY ? " selected" : ""}>${escapeHtml(year)}</option>`).join(""); const inFY = (value) => financialYearFor(value) === state.profitFY; const invoices = paidInvoices.filter((record) => inFY(profitInvoiceDate(record))); const fyExpenses = expenses.filter((expense) => inFY(expense.date)); const income = invoices.reduce((sum, record) => sum + Number(record.job.quote || 0) / 1.1, 0); const referral = invoices.reduce((sum, record) => markupAmountExGst(record.job) + sum, 0); const totalExpenses = fyExpenses.reduce((sum, expense) => sum + Number(expense.amount || 0), 0); const profit = income - referral - totalExpenses; const metrics = [["income","Paid income",income,`${invoices.length} paid invoice${invoices.length === 1 ? "" : "s"}`],["referral","Referral payable",referral,`${invoices.filter((record) => markupAmountExGst(record.job) > 0).length} invoice${invoices.filter((record) => markupAmountExGst(record.job) > 0).length === 1 ? "" : "s"}`],["expense","Expenses",totalExpenses,`${fyExpenses.length} expense${fyExpenses.length === 1 ? "" : "s"}`],["profit","Net profit",profit,"Before GST"]]; $("#profit-metrics").innerHTML = metrics.map(([style,label,amount,note]) => `<article class="profit-metric ${style}"><span>${label} (ex. GST)</span><strong>${currency(amount)}</strong><small>${escapeHtml(note)}</small></article>`).join(""); const max = Math.max(income, referral, totalExpenses, Math.abs(profit), 1); $("#profit-bars").innerHTML = metrics.map(([style,label,amount]) => `<div class="profit-bar-row"><span>${escapeHtml(label)}</span><div class="profit-bar"><i class="${style}" style="width:${Math.min(100, Math.abs(amount) / max * 100)}%"></i></div><strong>${currency(amount)}</strong></div>`).join(""); renderQuarterSummary(invoices, fyExpenses); const body = $("#expense-body"); body.innerHTML = fyExpenses.sort((a,b) => String(b.date).localeCompare(String(a.date))).map((expense) => `<tr><td>${escapeHtml(expense.date)}</td><td>${escapeHtml(expense.category)}</td><td>${escapeHtml(expense.description || "—")}</td><td class="money">${expense.bankAmount ? currency(expense.bankAmount) : "—"}</td><td class="money">${currency(expense.amount)}</td><td class="expense-actions"><button class="expense-delete" type="button" data-delete-expense="${escapeHtml(expense.id)}">Delete</button></td></tr>`).join(""); $("#expense-empty").style.display = fyExpenses.length ? "none" : "block"; }
function renderAccounting() { const select = $("#accounting-fy"); if (!select) return; const records = activeRecords(); const paid = records.filter((record) => record.job.status === "invoiced" && record.job.payment === "paid"); const unpaid = records.filter((record) => record.job.status === "invoiced" && record.job.payment !== "paid"); const expenses = profitExpenses(); const years = [...new Set([currentFinancialYear(), ...paid.map(profitInvoiceDate).map(financialYearFor), ...unpaid.map((record) => financialYearFor(record.job.invoiceDate)), ...expenses.map((expense) => financialYearFor(expense.date))].filter(Boolean))].sort().reverse(); if (!years.includes(state.accountingFY)) state.accountingFY = years.includes(currentFinancialYear()) ? currentFinancialYear() : years[0]; select.innerHTML = years.map((year) => `<option value="${escapeHtml(year)}"${year === state.accountingFY ? " selected" : ""}>${escapeHtml(year)}</option>`).join(""); const inFY = (value) => financialYearFor(value) === state.accountingFY; const paidInvoices = paid.filter((record) => inFY(profitInvoiceDate(record))); const unpaidInvoices = unpaid.filter((record) => inFY(record.job.invoiceDate)); const fyExpenses = expenses.filter((expense) => inFY(expense.date)); const income = paidInvoices.reduce((sum, record) => sum + Number(record.job.quote || 0) / 1.1, 0); const referral = paidInvoices.reduce((sum, record) => sum + markupAmountExGst(record.job), 0); const totalExpenses = fyExpenses.reduce((sum, expense) => sum + Number(expense.amount || 0), 0); const netProfit = income - referral - totalExpenses; const unpaidTotal = unpaidInvoices.reduce((sum, record) => sum + Number(record.job.quote || 0), 0); const metrics = [["income","Paid income (ex. GST)",income,`${paidInvoices.length} paid invoice${paidInvoices.length === 1 ? "" : "s"}`],["referral","Referral payable (ex. GST)",referral,`${paidInvoices.filter((record) => markupAmountExGst(record.job) > 0).length} invoice${paidInvoices.filter((record) => markupAmountExGst(record.job) > 0).length === 1 ? "" : "s"}`],["expense","Expenses (ex. GST)",totalExpenses,`${fyExpenses.length} transaction${fyExpenses.length === 1 ? "" : "s"}`],["profit","Net profit (ex. GST)",netProfit,"Before GST"],["expense","Unpaid invoices (inc. GST)",unpaidTotal,`${unpaidInvoices.length} invoice${unpaidInvoices.length === 1 ? "" : "s"}`]]; $("#accounting-metrics").innerHTML = metrics.map(([style,label,amount,note]) => `<article class="profit-metric ${style}"><span>${label}</span><strong>${currency(amount)}</strong><small>${escapeHtml(note)}</small></article>`).join(""); const barMetrics = [["income","Paid income",income],["referral","Referral payable",referral],["expense","Expenses",totalExpenses],["profit","Net profit",netProfit]]; const max = Math.max(income, referral, totalExpenses, Math.abs(netProfit), 1); $("#accounting-bars").innerHTML = barMetrics.map(([style,label,amount]) => `<div class="profit-bar-row"><span>${escapeHtml(label)}</span><div class="profit-bar"><i class="${style}" style="width:${Math.min(100, Math.abs(amount) / max * 100)}%"></i></div><strong>${currency(amount)}</strong></div>`).join(""); const outstandingText = unpaidInvoices.length ? `${unpaidInvoices.length} invoice${unpaidInvoices.length === 1 ? " is" : "s are"} awaiting payment.` : "No invoices are awaiting payment."; $("#accounting-outstanding-summary").innerHTML = `<b>${unpaidInvoices.length ? "!" : "✓"}</b><span><strong>${currency(unpaidTotal)} outstanding</strong><br>${outstandingText}</span>`; const categories = new Map(); fyExpenses.forEach((expense) => { const name = expense.category || "Uncategorised"; const current = categories.get(name) || { count:0, bank:0, amount:0 }; current.count += 1; current.bank += Number(expense.bankAmount ?? expense.amount ?? 0); current.amount += Number(expense.amount || 0); categories.set(name,current); }); const categoryRows = [...categories.entries()].sort((a,b) => b[1].amount - a[1].amount); $("#accounting-category-body").innerHTML = categoryRows.map(([name, totals]) => `<tr><td>${escapeHtml(name)}</td><td>${totals.count}</td><td class="money">${currency(totals.bank)}</td><td class="money"><strong>${currency(totals.amount)}</strong></td></tr>`).join(""); $("#accounting-category-empty").style.display = categoryRows.length ? "none" : "block"; $("#accounting-unpaid-body").innerHTML = unpaidInvoices.sort((a,b) => String(b.job.invoiceDate).localeCompare(String(a.job.invoiceDate))).map((record) => `<tr><td>${escapeHtml(record.job.invoiceNumber || record.job.number || "—")}</td><td class="address">${escapeHtml(record.address || "—")}</td><td>${escapeHtml(record.job.invoiceDate || "—")}</td><td class="money"><strong>${currency(record.job.quote)}</strong></td></tr>`).join(""); $("#accounting-unpaid-empty").style.display = unpaidInvoices.length ? "none" : "block"; }
