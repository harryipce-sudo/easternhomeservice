...
const state = { records: [], search: "", sort: "newest", view: savedView, layout: "board", selected: null, drawerStage: "quoted", invoiceFilter: "all", profitFY: "", expandedGroups: new Set(), customBoards: [], boardOrder: [], hiddenBoards: [], boardSettings: null };
...
function render() { renderJobs(); renderBoard(); renderBoardTargets(); renderCalendar(); renderClients(); renderQuotes(); renderArchive(); renderProfit(); }
...
function renderQuotes() {
  ...
  const pendingTotal = pendingForInvoice.reduce((sum, record) => sum + record.job.quote, 0);
  ...
  $("#invoice-filter-note").innerHTML = filterLabel ? `Showing <strong>${filterLabel}</strong> · <button type="button" data-invoice-filter="all">Show all invoices</button>` : "";
