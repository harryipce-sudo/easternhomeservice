...
function renderQuotes() {
  ...
  $("#invoices-body").innerHTML = invoices.map((record) => `<tr data-id="${escapeHtml(record.id)}"><td>${escapeHtml(record.job.invoiceNumber || "—")}</td><td>${escapeHtml(record.job.number)}</td><td>${escapeHtml(record.address)}</td><td>${escapeHtml(record.job.invoiceDate || "—")}</td><td>${escapeHtml(record.job.payment === "paid" ? record.job.paymentDate || "—" : "—")}</td><td>${paymentButton(record)}</td><td>${currency(record.job.quote)}</td></tr>`).join("") || `<tr><td colspan="7">No invoices recorded yet.</td></tr>`;
}
