...
function renderQuotes() {
  ...
  const referralPayable = (record) => record.job.payment === "paid" ? markupAmountExGst(record.job) * 1.1 : 0;
  ...
  $("#invoice-summary").innerHTML = `<article class="invoice-metric unpaid"><span>Unpaid invoices</span><strong>${unpaid.length} job${unpaid.length === 1 ? "" : "s"}</strong><b>${currency(unpaidTotal)}</b></article><article class="invoice-metric paid"><span>Paid invoices</span><strong>${paid.length} job${paid.length === 1 ? "" : "s"}</strong><b>${currency(paidTotal)}</b></article><article class="invoice-metric paid"><span>Referral payable (inc. GST)</span><strong>${paidReferralCount} paid invoice${paidReferralCount === 1 ? "" : "s"}</strong><b>${currency(paidReferralTotal)}</b></article><div class="invoice-payment-bar" aria-label="${unpaid.length} unpaid and ${paid.length} paid invoices"><span class="unpaid" style="width:${(unpaid.length / total) * 100}%"></span><span class="paid" style="width:${(paid.length / total) * 100}%"></span></div><div class="invoice-bar-legend"><span><i class="unpaid"></i>Unpaid: ${unpaid.length}</span><span><i class="paid"></i>Paid: ${paid.length}</span></div>`;
  $("#invoices-body").innerHTML = orderedInvoices.map((record) => { const key = invoiceKey(record.job.invoiceNumber); const issue = reviewIssue(record); const issueClass = !key ? "invoice-review-missing" : invoiceCounts.get(key) > 1 ? "invoice-review-duplicate" : issue ? "invoice-review-odoo" : ""; return `<tr class="${issueClass}" data-id="${escapeHtml(record.id)}"><td>${escapeHtml(record.job.invoiceNumber || "—")}${issue ? `<span class="invoice-review-badge">${escapeHtml(issue)}</span>` : ""}</td><td>${escapeHtml(record.job.number)}</td><td>${escapeHtml(record.address)}</td><td>${escapeHtml(record.job.invoiceDate || "—")}</td><td>${escapeHtml(record.job.payment === "paid" ? record.job.paymentDate || "—" : "—")}</td><td>${paymentButton(record)}</td><td>${record.job.payment === "paid" && referralPayable(record) > 0 ? currency(referralPayable(record)) : "—"}</td><td>${currency(record.job.quote)}</td></tr>`; }).join("") || `<tr><td colspan="8">No invoices recorded yet.</td></tr>`;
}
