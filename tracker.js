...
function renderQuotes() {
  ...
  $("#invoice-summary").innerHTML = `${metric("pending", "pending", "Pending for invoice", pendingForInvoice.length, pendingTotal)}${metric("unpaid", "unpaid", "Unpaid invoices", unpaid.length, unpaidTotal)}${metric("paid", "paid", "Paid invoices", paid.length, paidTotal)}${metric("unpaid-referral", "unpaid", "Unpaid referral payable (inc. GST)", unpaidReferralCount, unpaidReferralTotal, "invoice")}${metric("paid-referral", "paid", "Paid referral payable (inc. GST)", paidReferralCount, paidReferralTotal, "invoice")}<div class="invoice-payment-bar" aria-label="${unpaid.length} unpaid and ${paid.length} paid invoices"><span class="unpaid" style="width:${(unpaid.length / total) * 100}%"></span><span class="paid" style="width:${(paid.length / total) * 100}%"></span></div><div class="invoice-bar-legend"><span><i class="unpaid"></i>Unpaid: ${unpaid.length}</span><span><i class="paid"></i>Paid: ${paid.length}</span></div>`;
