...
function renderQuotes() { const records = activeRecords(); $("#quotes-body").innerHTML = records.map((record) => `<tr data-id="${escapeHtml(record.id)}"><td>${escapeHtml(record.job.number)}</td><td>${escapeHtml(record.address)}</td><td>${escapeHtml(record.job.detail || "—")}</td><td>${currency(record.job.quote)}</td><td>${tag(record.job.status)}</td></tr>`).join("") || `<tr><td colspan="5">No saved quotes yet.</td></tr>`; $("#invoices-body").innerHTML = records.filter((record) => record.job.invoiceNumber || record.job.status === "completed" || record.job.status === "invoiced").map((record) => `<tr data-id="${escapeHtml(record.id)}"><td>${escapeHtml(record.job.invoiceNumber || "—")}</td><td>${escapeHtml(record.job.number)}</td><td>${escapeHtml(record.address)}</td><td>${escapeHtml(record.job.invoiceDate || "—")}</td><td>${tag(record.job.payment)}</td><td>${currency(record.job.quote)}</td></tr>`).join("") || `<tr><td colspan="6">No invoices recorded yet.</td></tr>`; }
